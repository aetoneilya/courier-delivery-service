...
public class CouriersService {
    ...
    public GetCourierMetaInfoResponse getCourierMetaInfo(Long courierId, LocalDate startDate, LocalDate endDate) {
        ...
        List<Integer> earnings = courierRepository.getEarnedMoney(courierId, startDate.atTime(0,0).atOffset(ZoneOffset.UTC), endDate.atTime(0,0).atOffset(ZoneOffset.UTC));
        Integer sumEarnings = null, rating = null;

        if (earnings.size() != 0) {
            sumEarnings = earnings.stream().mapToInt(i -> i).sum() * config.couriersEarning().get(courier.courierType());
            rating = (earnings.size() / (Period.between(startDate, endDate).getDays() * 24)) * config.couriersRating().get(courier.courierType());
        }
